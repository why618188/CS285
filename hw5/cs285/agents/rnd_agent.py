...
class RNDAgent(DQNAgent):
    ...
    def update(
        ...
        with torch.no_grad():
            ...
            rnd_error = nn.functional.mse_loss(self.rnd_net(observations), self.rnd_target_net(observations),
                                               reduction='none').mean(dim=1)
            assert rnd_error.shape == rewards.shape, (rnd_error.shape, rewards.shape)

        reward = rewards + rnd_error

        metrics = super().update(observations, actions, reward, next_observations, dones, step)

        # Update the RND network.
        rnd_loss = self.update_rnd(observations)
        metrics["rnd_loss"] = rnd_loss

        metrics["original_reward"] = rewards.float().mean().item()
        metrics["rnd_error"] = rnd_error.float().mean().item()
        metrics["reward"] = reward.float().mean().item()
